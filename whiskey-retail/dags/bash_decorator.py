...
@dag(schedule=None, start_date=pendulum.datetime(2023, 1, 1, tz="UTC"), is_paused_upon_creation=False, catchup=False)
def bash_decorator():
    @task.bash
    ...
    sleep_in(day="{{ dag_run.start_date.strftime('%A').lower() }}")
    # [END howto_decorator_bash_conditional]

    # [START howto_decorator_bash_parametrize]
    @task.bash(env={"BASE_DIR": "{{ dag_run.start_date.strftime('%Y/%m/%d') }}"}, append_env=True)
    ...
    def _get_files_in_cwd() -> list[str]:
        ...
        dir_contents = Path.cwd().glob("root/airflow/dags/*.py")
